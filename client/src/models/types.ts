...
export type EventCard = { id: number; title: string; category: string; tags: string; startAt: string; location: string; capacity: number; imageUrl: string; description: string; ownerId: number; ownerName: string; participants: number; joined: boolean; status: 'DRAFT' | 'OPEN' | 'CLOSED' };
...
export type EventPayload = Omit<EventCard, 'id' | 'ownerName' | 'participants' | 'joined' | 'status'>;
